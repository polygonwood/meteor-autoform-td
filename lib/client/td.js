...
Template.afTempusDominusDateTimePicker.onRendered(function afTempusDominusDateTimePickerOnRendered() {
    ...
    // when change event is triggered inform autoform by change event on input field
    this.picker.subscribe(TempusDominus.Namespace.events.change, (e) => {
        ...
        // prevent initial load to fire change event
        if (!this.initial) this.$($input).change();
        else this.initial = false;
        // $input.dispatchEvent(new Event('change')); // this is not correct ?
    });
